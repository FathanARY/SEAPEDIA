...
const roleOptions = [
  {
    value: 'BUYER',
    label: 'Pembeli',
    description: 'Beli produk, kelola keranjang, dan lacak pesanan.',
    icon: (
  ...
  },
  {
    value: 'SELLER',
    label: 'Penjual',
    description: 'Buat toko, kelola produk, dan proses pesanan.',
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    value: 'DRIVER',
    label: 'Pengirim',
    description: 'Ambil pekerjaan pengiriman dan selesaikan pesanan.',
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 md:p-12 bg-surface-container-lowest relative overflow-y-auto">
        ...
        <div className="w-full max-w-lg relative py-8">
          <div className="mb-8 mt-8 lg:mt-0">
            <h2 className="text-3xl font-bold text-on-surface mb-2">Buat Akun Baru</h2>
            <p className="text-base text-on-surface-variant">Bergabunglah dengan SEAPEDIA untuk mulai berdagang.</p>
          </div>
          
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-bold tracking-wider text-on-surface mb-1 uppercase" htmlFor="reg-username">Username</label>
                <input 
                  id="reg-username"
              ...
              <div>
                <label className="block text-sm font-bold tracking-wider text-on-surface mb-1 uppercase" htmlFor="reg-email">Email</label>
                <input 
                  id="reg-email"
                  className="w-full px-4 py-2 bg-surface border border-outline-variant rounded-lg focus:border-primary focus:ring-2 focus:ring-primary/20 transition-colors text-sm" 
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-bold tracking-wider text-on-surface mb-1 uppercase" htmlFor="reg-password">Password</label>
                <input 
                  id="reg-password"
                  className="w-full px-4 py-2 bg-surface border border-outline-variant rounded-lg focus:border-primary focus:ring-2 focus:ring-primary/20 transition-colors text-sm" 
                  placeholder="••••••••" 
              ...
              <div>
                <label className="block text-sm font-bold tracking-wider text-on-surface mb-1 uppercase" htmlFor="reg-confirm">Konfirmasi</label>
                <input 
                  id="reg-confirm"
                  className="w-full px-4 py-2 bg-surface border border-outline-variant rounded-lg focus:border-primary focus:ring-2 focus:ring-primary/20 transition-colors text-sm" 
                  placeholder="••••••••" 
            ...
            {/* Role selection */}
            <div className="mt-6">
              <label className="block text-sm font-bold tracking-wider text-on-surface mb-2 uppercase">
                Pilih Peran <span className="text-on-surface-variant font-normal lowercase">(bisa pilih lebih dari satu)</span>
              </label>
                ...
                {roleOptions.map((role) => (
                  ...
                  >
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors ${
                      selectedRoles.includes(role.value) ? 'bg-primary text-on-primary' : 'bg-surface-container-high text-on-surface-variant'
                    }`}>
                      {role.icon}
                    </div>
                    <div>
                      <p className={`font-bold text-sm ${selectedRoles.includes(role.value) ? 'text-primary' : 'text-on-surface'}`}>{role.label}</p>
                      <p className="text-xs text-on-surface-variant mt-0.5">{role.description}</p>
                    </div>
                    <div className="ml-auto">
                      <div className={`w-5 h-5 rounded flex items-center justify-center transition-colors ${
                        selectedRoles.includes(role.value) ? 'bg-primary border-primary' : 'border-2 border-outline-variant'
                      }`}>
                        {selectedRoles.includes(role.value) && (
            ...
            {error && (
              <p className="text-sm text-error bg-error-container/20 px-4 py-2 rounded-lg mt-4">{error}</p>
            )}
          ...
          <p className="mt-8 text-center text-sm text-on-surface-variant">
            Sudah punya akun? 
            <Link href="/masuk" className="text-sm font-bold tracking-wider text-primary hover:underline ml-2">Masuk</Link>
          </p>
        </div>
      </div>
    </div>
